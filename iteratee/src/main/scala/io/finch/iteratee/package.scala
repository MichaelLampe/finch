...
package object iteratee extends IterateeInstances {
  ...
  implicit def encodeJsonEnumerator[F[_]: ConcurrentEffect, A](implicit
    A: Encode.Json[A],
    TE: ToEffect[Future, F]
  ): EncodeStream.Json[F, Enumerator, A] =
    new EncodeNewLineDelimitedEnumerator[F, A, Application.Json]

  implicit def encodeSseEnumerator[F[_]: ConcurrentEffect, A](implicit
  ...
  implicit def encodeTextEnumerator[F[_]: ConcurrentEffect, A](implicit
    A: Encode.Text[A],
    TE: ToEffect[Future, F]
...
trait IterateeInstances {
  ...
  protected final class EncodeNewLineDelimitedEnumerator[F[_]: ConcurrentEffect, A, CT <: String](implicit
    A: Encode.Aux[A, CT],
    TE: ToEffect[Future, F]
  ...
  implicit def encodeBufEnumerator[F[_]: ConcurrentEffect, CT <: String]: EncodeStream.Aux[F, Enumerator, Buf, CT] =
